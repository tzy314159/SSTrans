...
def initialize(config, train_data_set_len):
    ...
    optimizer_encoder = BertAdam(model.parameters(), lr=config.learning_rate, warmup=config.warmup, t_total=t_total) 
...
def training(local_rank, config=None, **kwargs):
    ...
    train_data_set, eval_data_set = get_dataflow(config) # dataflow
    train_loader = get_data_loader(config, is_train=True, data_set=train_data_set)
    valid_loader = get_data_loader(config, is_train=False, data_set=eval_data_set) # dataloader
