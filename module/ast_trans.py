...
class FastASTTrans(BaseTrans):
    def __init__(self, src_vocab_size, tgt_vocab_size, hidden_size, par_heads, num_heads,
        ...
        self.priority_embedding= Embeddings(hidden_size=hidden_size,
                                        vocab_size=src_vocab_size,
                                        dropout=dropout,
                                        with_pos=False)
...
class FastASTEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        for i, layer in enumerate(self.layers):
            output = layer(output, start_nodes, self.end_nodes, rel_q, rel_k, rel_v,node_max)
...
class FastASTEncoderLayer(nn.Module): # this is encoder unit
    ...
    def forward(self, src, start_nodes, end_nodes, rel_q, rel_k, rel_v,node_maxtrix):
        ...
        src, attn_weights = self.sublayer[0](src, lambda x: self.self_attn(x, x, x, start_nodes, end_nodes,
                                                                           rel_q, rel_k, rel_v,node_maxtrix))
