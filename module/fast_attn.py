...
from module.torch_gcn import GCN,SAGE
...
class FastMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, start_nodes=None, end_nodes=None,
                rel_q=None, rel_k=None, rel_v=None,node_maxtrix=None):
        ...
        query, key, value = [transpose_for_scores(l(x), self.h) for l, x in zip(self.linear_layers,
                                                                                (query, key, value))]
        q_shape = query.shape
        all_src = torch.tensor([],device = query.device)
        for id,value1 in enumerate(query):
            value1 = value1.reshape(200,256)            
            x= self.gcn(value1,node_maxtrix[id])

            x = x.reshape(1,q_shape[1],q_shape[2],q_shape[3])
            all_src= torch.cat([all_src,x],dim=0)
        query =  torch.tanh(all_src.reshape(q_shape)*self.lambda_1)

        q_shape = key.shape
        all_src = torch.tensor([],device = key.device)
        for id,value1 in enumerate(key):
            value1 = value1.reshape(200,256)
            x= self.gcn(value1,node_maxtrix[id])
            x = x.reshape(1,q_shape[1],q_shape[2],q_shape[3])

            all_src= torch.cat([all_src,x],dim=0)
        key = torch.tanh(all_src.reshape(q_shape)*self.lambda_1)


        q_shape = value.shape
        all_src = torch.tensor([],device = value.device)
        for id,value1 in enumerate(value):
            value1 = value1.reshape(200,256)
            ...
            all_src= torch.cat([all_src,x],dim=0)
        value =  torch.tanh(all_src.reshape(q_shape)*self.lambda_1)
    ...
    def rel_attn(q, k, v, start_nodes, end_nodes, rel_q, rel_k, rel_v):
        ...
        # g_encoder = g_encoder.contiguous().view(-1, d_k)
        ...
        # g_context = torch.embedding(g_encoder,query_indexes)
        ...
        # g_context = g_context.view(batch_size, num_heads, -1, seq_len, d_k)
